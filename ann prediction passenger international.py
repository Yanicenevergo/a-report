...
def main(year):
    ...
    if year<2012 or year>2030:
        raise ValueError('year must be between 2012 and 2030')
    
    idx = year-2012
    
    df=pd.read_excel('data.xlsx')
    df = df[['t','gdp','cpi','am','demand_t','demand_t+1','demand_t-1']]
    ...
    demands_abs = list(df['demand_t+1'].dropna())[:27]

    X_train = data_X.iloc[:26,:] #train data
    y_train = data_y[:26]
    ...
    X_test = scaler1.transform(data_X.iloc[26,:].values.reshape(1,-1)).squeeze()

    y_train = scaler2.fit_transform(y_train.values.reshape(-1,1)).squeeze()
    
    #model with grid search
    model_base=MLPRegressor(random_state=1)
    ...
    gdp=data_X['gdp_diff'][27:].values
    cpi=data_X['cpi_diff'][27:].values
    am=data_X['am_diff'][27:].values
