...
def main(year):
    ...
    if year<2015 or year>2030:
        raise ValueError('year must be between 2015 and 2030')
    ...
    X_train = data_X.iloc[:29,:] #train data
    y_train = data_y[:29]
    ...
    X_test = scaler1.transform(data_X.iloc[29,:].values.reshape(1,-1)).squeeze()
    ...
    gdp=data_X['gdp_diff'][30:].values
    cpi=data_X['cpi_diff'][30:].values
    am=data_X['am_diff'][30:].values
